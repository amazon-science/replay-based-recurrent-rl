...
from misc.eval import evaluate_policy_on_all_tasks_mtl
...
# General params
parser.add_argument('--multi_task', type=str2bool, default=True, choices=[True])
...
if __name__ == "__main__":
    ...
    if args.nb_tasks == 1:
        # these dont make sense in STL
        assert not any(x in args.method_config for x in ['ID', 'EMB', 'MH'])
    ...
    evaluate_policy_on_all_tasks_mtl(alg, args, envs, total_timesteps, total_updates, 
    ...
    while total_timesteps < args.total_timesteps:
        ...
        if updates_since_logging >= args.log_freq:
            ...
            logger.dump_tabular()
            print(("updates: %d Episode Num: %d  Episode T: %d Reward: %f") %
                    (total_updates, total_episodes, data['episode_timestep'],
                    data['episode_reward']))
        ...
        if updates_since_eval >= args.eval_freq:
            ...
            evaluate_policy_on_all_tasks_mtl(alg, args, envs, total_timesteps, 
                    total_updates, wrt_csv_eval=wrt_csv_eval, wandb=wandb)
            
        #######
        # save for every interval-th update or for the last epoch
        #######
        if (updates_since_saving >= args.save_freq or total_updates > args.total_updates - 1):
    ...
    evaluate_policy_on_all_tasks_mtl(alg, args, envs, total_episodes, total_updates,
            wrt_csv_eval=wrt_csv_eval, wandb=wandb)

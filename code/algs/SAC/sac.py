...
class SAC:
    ...
    def select_action(self, task_id, obs, previous_action, previous_reward, previous_obs, 
                      deterministic=False, eval_step=False, ret_context=False):
        ...
        with torch.no_grad():
            ...
            out = self.actor(task_id, obs, pre_act_rew, deterministic=deterministic,
                                 with_logprob=False, ret_context=ret_context)
    ...
    def compute_critic_loss(self, task_id, obs, next_obs, action, reward, mask, 
                            pre_act_rew, act_rew, last_time):
        '''
            Compute loss Q:
        ...
        with torch.no_grad():
                ...
                # backup: [B, 1]
                backup   = reward + self.gamma  * mask * (target_Q - self.alpha * logp_next_actions)

        # 2.  Get current Q estimates
        current_Q1, current_Q2, extras1, extras2 = self.critic(task_id, obs, action, pre_act_rew)

        # 3. Compute critic loss
        # even we picked min Q, we still need to backprob to both Qs
        critic_loss = ((current_Q1 - backup)**2).mean() + ((current_Q2 - backup)**2).mean()

        # Useful info for logging
        q_info = dict(Q1Vals=current_Q1.squeeze(-1).detach().cpu().numpy(),
    ...
    def train(self, 
        ...
        for it in range(iterations):
            ...
            if self.hist_len>0:
                previous_actions = torch.FloatTensor(data['previous_actions']).to(self.device)
                previous_rewards = torch.FloatTensor(data['previous_rewards']).to(self.device)
                ...
                hist_actions = torch.FloatTensor(data['current_actions']).to(self.device)
                hist_rewards = torch.FloatTensor(data['current_rewards']).to(self.device)
                ...
                pre_act_rew = [previous_actions, previous_rewards, previous_obs] #torch.cat([previous_action, previous_reward], dim = -1)
            else:
                act_rew = None
                pre_act_rew = None
            ...
            critic_loss, q_info = self.compute_critic_loss(
                    task_id, obs, next_obs, action, reward, mask, pre_act_rew, act_rew, last_time)
            ...
            if self.monitor_grads:
                critic_grads_epoch = accumulate_grads_epoch(self.critic, critic_grads_epoch)
                if last_time:
                    compute_grad1(self.critic)
                    critic_grad_summary = gradient_analysis(self.critic, critic_grads_epoch)
            if self.grad_clip:
            ...
            ########
            # Optimize the actor
            actor_loss, alout, pi_info = \
                    self.compute_loss_pi(task_id, obs, pre_act_rew, last_time)

            clear_backprops(self.actor)
            self.actor_optimizer.zero_grad()
            actor_loss.backward()
            if self.monitor_grads:
                actor_grads_epoch = accumulate_grads_epoch(self.critic, critic_grads_epoch)
                if last_time:
                    compute_grad1(self.actor)
                    actor_grad_summary = gradient_analysis(self.actor, actor_grads_epoch)
            if self.grad_clip:

...
def add_hooks(model: nn.Module) -> None:
    ...
    for layer in model.modules():
        if _layer_type(layer) in _supported_layers:
            print(f"adding hook on layer {_layer_type(layer)}")
            handles.append(layer.register_forward_hook(_capture_activations))
            handles.append(layer.register_backward_hook(_capture_backprops))
            ## NOTE: this will throw a warning (bc we have weight dictionaries)
            ## but essentially it's fine bc of the for loop.
...
def compute_grad1(model: nn.Module, loss_type: str = "mean") -> None:
    ...
    for layer in model.modules():
        ...
        if layer_type == "Linear":
            # NOTE: this is breaking for the transformer
            # needs the following gymnastics to account for the fact that we are
            # only feeding the last embedded token to the actor and critic:
            if len(A.shape) == 3:
                A = A[-1, :, :]
                B = B[-1, :, :]
